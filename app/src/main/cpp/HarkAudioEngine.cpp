...
static const double UI_CENTER_FREQS[16] = {250,  315,  400,  500,  630,  800,
...
HarkAudioEngine::HarkAudioEngine()
    : sampleRate(48000.0), mNoiseSuppressorL(48000.0),
      mNoiseSuppressorR(48000.0), mGestureDetector(48000.0),
      mEqLeft(NUM_UI_BANDS), mEqRight(NUM_UI_BANDS) {

  mMediaAudioQueue = std::make_unique<LockFreeQueue<float>>(
      48000 * 2 * 2); // 2 seconds of stereo float buffer
  ...
  updateWdrcParameters(-20.0f, 1.2f, -72.0f, 0.5f, 10.0f, 600.0f);
  updateDSPParameters();
}
...
void HarkAudioEngine::updateDSPParameters() {
  // LR4 Crossover setup
  const double xoFreqs[7] = {1500.0, 500.0,  4500.0, 250.0,
                             1000.0, 2500.0, 6000.0};
...
void HarkAudioEngine::recomputePrescriptionGains() {
  ...
  // 計算 Auto-Headroom (依照 EQ 設定自動衰減主音量，防止大訊號壓迫限制器而切波)
...
void HarkAudioEngine::setSituationalMode(SituationalMode mode) {
  ...
  case SituationalMode::TRANSPARENCY:
    ...
    mNoiseSuppressorL.setEnabled(false);
    mNoiseSuppressorR.setEnabled(false);
    break;
  case SituationalMode::CONVERSATION:
    // 提高下擴展門檻至 -40dBFS，比例設為 4:1 (0.25f)，在無人說話時強力消減
    // EarPods 等耳機的底噪
    updateWdrcParameters(-30.0f, 1.5f, -40.0f, 0.25f, 5.0f, 200.0f);
    mNoiseSuppressorL.setEnabled(true);
    mNoiseSuppressorR.setEnabled(true);
    break;
  case SituationalMode::OUTDOOR:
    // 戶外同樣採用 -40dBFS 搭配 4:1 比例來阻絕風噪與環境低頻噪音
    updateWdrcParameters(-25.0f, 1.3f, -40.0f, 0.25f, 5.0f, 200.0f);
    mNoiseSuppressorL.setEnabled(true);
    mNoiseSuppressorR.setEnabled(true);
    break;
  case SituationalMode::CINEMA:
    // 影音模式
    updateWdrcParameters(-15.0f, 1.1f, -65.0f, 0.4f, 20.0f, 600.0f);
    mNoiseSuppressorL.setEnabled(true);
    mNoiseSuppressorR.setEnabled(true);
    break;
...
HarkAudioEngine::onAudioReady(oboe::AudioStream * /*stream*/, void *audioData,
  ...
  for (int i = 0; i < numFrames * CHANNEL_COUNT; i += CHANNEL_COUNT) {
    ...
    if (mDcBlockerEnabled.load(std::memory_order_relaxed)) {
      float curInL = sL;
      float curInR = sR;
      sL = curInL - mDcLastInL + 0.995f * mDcLastOutL;
      sR = curInR - mDcLastInR + 0.995f * mDcLastOutR;
      mDcLastInL = curInL;
      mDcLastInR = curInR;
      mDcLastOutL = sL;
      mDcLastOutR = sR;
...
void HarkAudioEngine::getDiagnosticMetrics(float *outMetrics) {
  ...
  // 4. Input XRuns
  outMetrics[3] = mInputStream
                      ? static_cast<float>(mInputStream->getXRunCount().value())
                      : 0.0f;

  // 5. Output XRuns
  outMetrics[4] =
      mOutputStream ? static_cast<float>(mOutputStream->getXRunCount().value())
                    : 0.0f;
}
