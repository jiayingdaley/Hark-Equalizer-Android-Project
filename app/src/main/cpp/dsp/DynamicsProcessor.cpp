...
DynamicsProcessor::DynamicsProcessor() :
        ...
        mExpanderRatio(1.0f),
        mNoiseFloorDb(HarkDspConfig::DYNAMICS_NOISE_FLOOR_INIT),
        mExpanderThresholdDbPreset(-50.0f),
        mAlphaNoiseUp(HarkDspConfig::DYNAMICS_NOISE_ALPHA_UP),
        mAlphaNoiseDown(HarkDspConfig::DYNAMICS_NOISE_ALPHA_DOWN),
        mAttackCoeff(1.0f),
        mReleaseCoeff(1.0f),
        mCurrentGain(1.0f),
        mTargetGain(1.0f),
        mCounter(0),
        mUpdateInterval(HarkDspConfig::DYNAMICS_GAIN_UPDATE_INTERVAL_NORMAL),
        mEnvelope(0.0f),
        mKneeDb(HarkDspConfig::DYNAMICS_KNEE_DB) {}
...
void DynamicsProcessor::setParameters(float compressThresholdDb, float compressRatio, 
    ...
    if (attackMs < HarkDspConfig::DYNAMICS_FAST_ATTACK_THRESH_MS) {
        mUpdateInterval = HarkDspConfig::DYNAMICS_GAIN_UPDATE_INTERVAL_FAST;
    } else {
        mUpdateInterval = HarkDspConfig::DYNAMICS_GAIN_UPDATE_INTERVAL_NORMAL;
    }
...
float DynamicsProcessor::process(float inputSample) {
    ...
    if (mCounter++ >= mUpdateInterval) {
        ...
        if (mNoiseFloorDb > HarkDspConfig::DYNAMICS_NOISE_FLOOR_MAX) mNoiseFloorDb = HarkDspConfig::DYNAMICS_NOISE_FLOOR_MAX;
        if (mNoiseFloorDb < HarkDspConfig::DYNAMICS_NOISE_FLOOR_MIN) mNoiseFloorDb = HarkDspConfig::DYNAMICS_NOISE_FLOOR_MIN;

        // Dynamic expander threshold: preset ceiling or noise_floor + 5.0 dB
        float finalExpanderThresholdDb = mExpanderThresholdDbPreset;
        if (mExpanderThresholdDbPreset > HarkDspConfig::DYNAMICS_ADAPTIVE_DISABLE_THRESH) {
            float adaptiveThresholdDb = mNoiseFloorDb + HarkDspConfig::DYNAMICS_ADAPTIVE_OFFSET_DB;
    ...
    mCurrentGain = HarkDspConfig::DYNAMICS_GAIN_SMOOTH_CURR * mCurrentGain + HarkDspConfig::DYNAMICS_GAIN_SMOOTH_NEW * mTargetGain;
