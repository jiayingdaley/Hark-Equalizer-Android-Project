...
TransientSuppressor::TransientSuppressor() :
    ...
    mEnabled(true),
    mThresholdDb(HarkDspConfig::TS_THRESHOLD_DB),
    mMinLevelDb(HarkDspConfig::TS_MIN_LEVEL_DB),
    mAttenuationDb(HarkDspConfig::TS_ATTENUATION_DB),
    mHoldMs(HarkDspConfig::TS_HOLD_MS),
    mReleaseMs(HarkDspConfig::TS_RELEASE_MS),
    mFastCoeff(0.0f),
    mSlowCoeff(0.0f),
    mAttackCoeff(0.0f),
    mReleaseCoeff(0.0f),
...
void TransientSuppressor::updateCoefficients() {
    // Fast Env: ~1ms
    mFastCoeff = std::exp(-1.0f / (static_cast<float>(mSampleRate) * (HarkDspConfig::TS_FAST_ENV_MS / 1000.0f)));
    
    // Slow Env: ~100ms
    mSlowCoeff = std::exp(-1.0f / (static_cast<float>(mSampleRate) * (HarkDspConfig::TS_SLOW_ENV_MS / 1000.0f)));
    
    // Fast Attack: ~0.5ms to be extremely responsive but smooth to prevent clicks/pops
    mAttackCoeff = std::exp(-1.0f / (static_cast<float>(mSampleRate) * (HarkDspConfig::TS_ATTACK_MS / 1000.0f)));
