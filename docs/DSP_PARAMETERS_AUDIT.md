...
└─────────────────────────┬───────────────────────────────────┘
                          ▼  (以下 L/R 雙聲道對稱處理)
┌─────────────────────────────────────────────────────────────┐
│  [1] TransientSuppressor — 時域極速抑制                    │
│  └ 消除突發性尖銳脈衝噪音以保護耳朵                         │
└─────────────────────────┬───────────────────────────────────┘
                          ▼
┌─────────────────────────────────────────────────────────────┐
│  [2] NoiseSuppressor — Wiener SNR-Gate                      │
│  ├ 風噪高通：HighPass @150Hz (Q=0.7)                         │
│  ├ 5頻帶分析濾波器：500/1000/2000/3000/4000 Hz              │
│  ├ 能量追蹤：E = 0.98×E_prev + 0.02×|x|                    │
│  ├ 噪音地板：NF = 0.999×NF + 0.001×E (安靜期更新)           │
│  ├ SNR 增益：G = SNR/(SNR+2.0)，底限 0.20 (-14dB)           │
│  ├ 增益平滑：G_smooth = 0.992×G_prev + 0.008×G_target       │
│  └ 中頻語音權重：1k-3kHz 權重 ×2，其他 ×1                   │
└─────────────────────────┬───────────────────────────────────┘
                          ▼
┌─────────────────────────────────────────────────────────────┐
│  [3] Multi-band Crossover & WDRC + Prescription Mapping     │
│  ├ 映射: 16 UI Bands → 8 Internal WDRC Bands                │
│  ├ 增益: UI_Gain + Global_Offset (+8.0dB)                   │
│  ├ 低頻與高頻噪音管理: Band 0/1/7 具有高閥值壓制            │
│  └ 核心語音頻段放寬: Band 2-5 設為 -55dBFS, Ratio=1.5:1 (0.66)│
└─────────────────────────┬�┌─────────────────────────────────────────────────────────────┐
│  [7] 8-Band Symmetric LR4 Crossover Tree + 8x WDRC Bands    │
│                                                             │
│  對稱二元分頻樹拓撲 (Symmetric Binary Tree Topology):         │
│                        Input                                │
│                          │                                  │
│                 [XoverMid @1500Hz]                          │
│                  ├── low (<1500Hz)                          │
...
│                                    │     └── high → Band 7  │
│                                                             │
│  每個 WDRC 參數根據情境模式動態設定，且 Bands 2-5 強制放寬     │
│  Expander (Threshold=-55dBFS, Ratio=1.5:1)，B0/1/7 具有高閥值│
└─────────────────────────┬───────────────────────────────────┘
                          ▼
┌─────────────────────────────────────────────────────────────┐
│  [8] Output MPO Limiter (Maximum Power Output)              │
│  ├ Threshold: -4.5dBFS (保護使用者聽力防過載)                │
│  ├ Ratio: 20:1                                              │
