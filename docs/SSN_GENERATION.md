...
- 每題從 60 秒噪音中隨機取段，噪音比語音前後各多 500 ms（noise lead/trail），
  讓使用者先聽到噪音再出現語詞。
- **呈現餘裕與削波防護**：所有 trial 一律先套用固定 −6 dB 呈現餘裕
  （`PRESENTATION_GAIN_DB`，trial 間一致，故不影響 SNR 亦不影響 trial 間
  絕對級別的可比性）。混音以浮點求峰值後一次性量化；若峰值仍超出 16-bit
  範圍，才整段等比例縮小，且**該題的額外衰減量（dB）會寫入資料庫
  `ssn_test_records.norm_gain_db`**，並在結果頁與歷史詳情揭露
  （觸發題數與最大衰減）。SNR 不受衰減影響，但該些題目的絕對呈現級別
  相對其他題較低——論文報告時應一併揭露此欄位。
- 因 SNR 由數位混音精確控制，**與播放音量無關**，故測驗允許使用者
  先將音量調至最舒適音量（MCL）再開始；測驗開始後音量鍵鎖定。
